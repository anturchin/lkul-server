...
(async () => {
  try {
    ...
    const ssdUriMsk = await SsdUri.findOne({ title: "ССД МРГ" });
    const ssdUriKzn = await SsdUri.findOne({ title: "Казань препрод LKKUL1" });
    const ssdUriSpb = await SsdUri.findOne({ title: "Санкт-Петербург" });
    ...
    const regionKzn = await Region({
      ...
      ssdUri: ssdUriKzn._id,
    }).save();

    const regionSpb = await Region({
    ...
    await SsdUri.findOneAndUpdate(
      { _id: ssdUriKzn._id },
      { $set: { regionId: regionKzn._id } }
    );
    await SsdUri.findOneAndUpdate(
      { _id: ssdUriSpb._id },
      { $set: { regionId: regionSpb._id } }
    );
